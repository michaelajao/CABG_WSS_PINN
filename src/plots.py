...
def generate_all_plots(model: nn.Module, dataset: PatientData,
    ...
    """Generate all comparison plots, including per-vessel and training history."""
    model.eval()
    ...
    with torch.no_grad():
        for start in range(0, n_samples, batch_size):
            ...
            vel_pred_nondim = torch.cat(
                [outputs['u'], outputs['v'], outputs['w']], dim=1
            ).cpu().numpy()
...
def plot_full_patient_wss(patient_id: str, vessel_data: list,
    ...
    # Fixed across all patients and rheologies so panels stay comparable
    # between cases. 40 dynes/cm^2 is the disease threshold used throughout;
    # the error panel uses a tight 0-10 range so small residuals stay readable.
    vmax = 40.0
    error_vmax = 10.0
    ...
    for (x_idx, y_idx), plane_name in zip(planes, plane_names):
        ...
        # Absolute error
        sc3 = axes[2].scatter(x_plot, y_plot, c=wss_error, cmap='Reds', s=0.5,
...
#
# Paired Newtonian/Carreau-Yasuda comparison. Pass the common-subset metrics
# directory: the two rheologies are only comparable over the per-patient vessel
# subset they both cover, not the full-coverage metrics under reports/metrics:
